/**
 * HistoryTable.jsx
 * Displays a table of past sentiment analyses with filtering.
 */
        ...
        <table className="w-full">
          ...
          <tbody>
            {entries.map((entry, idx) => (
              <tr
                key={entry.id}
                className="border-b border-ink-700 hover:bg-ink-700 transition-colors duration-150"
              >
                <td className="px-5 py-4 text-gray-600 font-display text-xs">
                  {String(idx + 1).padStart(2, '0')}
                </td>
                <td className="px-5 py-4 max-w-xs">
                  <p className="text-gray-200 text-sm font-body truncate" title={entry.text}>
                    {entry.text}
                  </p>
                  {/* Mobile: show sentiment inline */}
                  <div className="sm:hidden mt-1">
                    <span className={SENTIMENT_TAG[entry.sentiment] || 'tag-neutral'}>
                      {entry.sentiment}
                    </span>
                  </div>
                </td>
                    ...
                    <div className="w-16 bg-ink-700 rounded-full h-1.5">
                      <div
                        className={`h-full rounded-full ${
                          entry.sentiment === 'Positive' ? 'bg-accent-lime' :
                          entry.sentiment === 'Negative' ? 'bg-accent-rose' : 'bg-accent-amber'
                        }`}
