...
export default function SentimentForm({ onResult, onLoading }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg =
        ...
        (err.code === 'ERR_NETWORK' ? 'Cannot connect to backend. Is the server running?' : 'Analysis failed. Please try again.');
      setError(msg);
      onResult(null);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="glass-card p-6 md:p-8">
      ...
      <textarea
        value={text}
        onChange={(e) => { if (e.target.value.length <= charLimit) setText(e.target.value); if (error) setError(''); }}
        onKeyDown={handleKeyDown}
        placeholder="Type or paste your text here..."
        rows={5}
        className="w-full bg-ink-900 border border-ink-600 rounded-xl px-4 py-3 text-gray-100 font-body text-sm placeholder-gray-600 focus:outline-none focus:border-accent-cyan focus:ring-1 focus:ring-accent-cyan transition-all duration-200 resize-none"
      />
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {EXAMPLE_PHRASES.map((phrase, i) => (
            <button
              key={i}
              onClick={() => { setText(phrase); setError(''); }}
              className="text-xs font-body text-gray-400 border border-ink-600 rounded-lg px-3 py-1.5 hover:border-accent-cyan hover:text-accent-cyan transition-all duration-200 text-left max-w-xs truncate"
            >
      ...
      <div className="mt-6 flex items-center justify-between gap-4">
        ...
        <div className="flex gap-3 ml-auto">
          {text && (
            <button onClick={() => { setText(''); setError(''); }} className="btn-ghost text-sm">Clear</button>
          )}
          <button
