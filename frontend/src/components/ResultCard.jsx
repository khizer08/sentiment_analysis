const SENTIMENT_CONFIG = {
  Positive: { tagClass: 'tag-positive', glowClass: 'glow-lime', borderColor: 'border-accent-lime', barColor: 'bg-accent-lime', emoji: '😊' },
  Negative: { tagClass: 'tag-negative', glowClass: 'glow-rose',  borderColor: 'border-accent-rose', barColor: 'bg-accent-rose', emoji: '😞' },
  Neutral:  { tagClass: 'tag-neutral',  glowClass: '',            borderColor: 'border-accent-amber', barColor: 'bg-accent-amber', emoji: '😐' },
};
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 bg-ink-700 rounded-full h-1.5 overflow-hidden">
        <div className={`h-full rounded-full ${color}`} style={{ width: `${Math.round(value * 100)}%`, transition: 'width 1s ease-out' }} />
      </div>
      <span className="text-xs text-gray-400 font-display w-10 text-right">{Math.round(value * 100)}%</span>
    </div>
  );
}
  ...
  return (
    <div className={`glass-card border-2 ${config.borderColor} ${config.glowClass} p-6 md:p-8 animate-slide-up`}>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs font-body mt-1">{new Date(result.analyzedAt).toLocaleTimeString()}</p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-5 p-3 bg-ink-900 rounded-xl border border-ink-600">
        ...
        <p className="text-gray-300 text-sm font-body leading-relaxed line-clamp-3">"{result.text}"</p>
      </div>

      {/* Per-model breakdown */}
      {result.models && result.models.length > 0 && (
        <div className="mb-5 space-y-2">
          <p className="section-label mb-3">Model Breakdown</p>
          {result.models.map((m) => (
            <div key={m.model} className="flex items-center gap-3">
              <span className="text-xs text-gray-500 font-body w-16">{m.model}</span>
              <div className="flex-1 bg-ink-700 rounded-full h-1.5 overflow-hidden">
                <div
                  className={`h-full rounded-full ${m.score >= 0.05 ? 'bg-accent-lime' : m.score <= -0.05 ? 'bg-accent-rose' : 'bg-accent-amber'}`}
                  style={{ width: `${Math.min(100, Math.max(0, ((m.score + 1) / 2) * 100))}%`, transition: 'width 1s ease-out' }}
                />
              </div>
              <span className="text-xs text-gray-400 font-display w-14 text-right">
