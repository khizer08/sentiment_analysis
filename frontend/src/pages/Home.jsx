...
const features = [
  { icon: "🧠", title: "Multi-Model NLP Engine", desc: "Combines VADER, TextBlob, and BERT to deliver more accurate and reliable sentiment predictions." },
  { icon: "📊", title: "Aggregated Confidence",  desc: "Final sentiment is computed using the average score and confidence across all models." },
  { icon: "⚡", title: "Real-time Analysis",     desc: "Fast results via a REST API connecting React, Node.js, and a Python NLP microservice." },
  { icon: "📜", title: "History Tracking",       desc: "All past analyses are stored server-side so you can compare and review them later." },
];

const stack = ["React.js","Vite","Tailwind CSS","Node.js","Express.js","Python","VADER","TextBlob","BERT (Transformers)","REST API"];
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
      ...
      <div className="text-center mb-20 animate-fade-in">
        ...
        <p className="text-gray-400 text-lg sm:text-xl max-w-2xl mx-auto font-body leading-relaxed mb-10">
          A full-stack sentiment analysis system powered by multiple NLP models. Paste any text and get instant classification as Positive, Negative, or Neutral — using a hybrid approach with improved accuracy.
        </p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link to="/analyzer" className="btn-primary text-base px-8 py-4 glow-cyan">Start Analyzing →</Link>
          <Link to="/about" className="btn-ghost text-base px-8 py-4">How it works</Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-20">
        {features.map((f, i) => (
          <div key={i} className="glass-card p-6 hover:border-ink-500 transition-colors duration-300">
      ...
      <div className="glass-card p-6 md:p-10 mb-16">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-3 md:gap-2 flex-wrap">
          {[
            ...
            { label: "→",                           sub: "HTTP POST",        color: "text-gray-600", noBox: true },
            { label: "Python NLP (VADER+TextBlob+BERT)", sub: "NLP Service", color: "border-accent-amber text-accent-amber" },
          ].map((item, i) =>
            item.noBox ? (
      ...
      <div className="text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {stack.map((tech) => (
            <span key={tech} className="bg-ink-800 border border-ink-600 rounded-lg px-4 py-2 text-sm font-display text-gray-300">{tech}</span>
          ))}
        </div>
      </div>
    </div>
